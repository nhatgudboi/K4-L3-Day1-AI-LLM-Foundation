...
def run_assistant(
    ...
    while True:
        ...
        cost_stats = estimate_cost(full_prompt, reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    table += "-" * 120 + "\n"
    for r in results:
        p = truncate(r.get("prompt", ""))
        o = truncate(r.get("gpt4o_response", ""))
        m = truncate(r.get("mini_response", ""))
        ol = f"{r.get('gpt4o_latency', 0):.2f}s"
        ml = f"{r.get('mini_latency', 0):.2f}s"
        table += f"{p:<43} | {o:<43} | {m:<43} | {ol:<14} | {ml:<12}\n"
    return table
